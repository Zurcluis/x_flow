...
import { initialTimeBookModels } from "@/lib/demo-data/timebook-b2b-data";

export default function TimeBookPage() {
  const [selectedModelId, setSelectedModelId] = useState<string>("tb-bmw-m4");

  const activeModel =
    initialTimeBookModels.find((m) => m.id === selectedModelId) ||
    initialTimeBookModels[0];
  ...
  return (
    <div className="flex flex-col gap-6 p-6 max-w-7xl mx-auto w-full">
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2 border-b border-white/[0.06]">
        {initialTimeBookModels.map((model) => (
          <button
            key={model.id}
