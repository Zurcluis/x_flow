...
import { initialBaysData, initialAppointmentsData } from "@/lib/demo-data/appointments-data";
import { calculateWeeklyCapacity } from "@/domains/calendar/capacity-calculator";

export default function CalendarPage() {
  const [bays] = useState(initialBaysData);
  const [appointments] = useState(initialAppointmentsData);
  const [selectedDate] = useState("2026-08-28");

  // Calculate deterministic capacity metrics
  const capacity = calculateWeeklyCapacity(appointments, 4, 40);
